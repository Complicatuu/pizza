...
try {
	...
	if ($q !== '') {
    	$stmt = $con->prepare("SELECT * FROM produse WHERE nume LIKE :q OR descriere LIKE :q");
    	$searchTerm = "%$q%";
...
<div id="wrapper" class="fade-in">
    ...
    <div id="main" class="container mt-5">
		...
		<!--Cautare Produse-->
		 <form method="GET" class="mb-4">
    		<div class="input-group">
        		<input type="text" name="q" class="form-control" placeholder="Caută produse..." value="<?= isset($_GET['q']) ? htmlspecialchars($_GET['q']) : '' ?>">
        ...
        <?php if ($q !== ''): ?>
            <p class="text-muted">Rezultatele pentru: <strong><?= htmlspecialchars($q) ?></strong></p>
        <?php endif; ?>


		<?php if (empty($produse)): ?>
    		<div class="alert alert-warning">Nu am găsit niciun produs care să corespundă căutării.</div>
		<?php endif; ?>
